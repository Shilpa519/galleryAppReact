...
class Gallery extends Component {
  ...
  getFilteredImages = () => {
    ...
    const filteredImage = imagesList.filter(
      selectedImage => selectedImage.imageUrl === activeId,
    )
  ...
  render() {
    const {activeId} = this.state
    let filteredImage = this.getFilteredImages()
    filteredImage = imagesList.filter(eachItem => eachItem.id === activeId)
        ...
        <ul className="thumbnail-container">
          {imagesList.map(eachItem => (
            <ThumbnailItem
              key={eachItem.id}
              imagesItem={eachItem}
              selectedItem={this.clickImageItem}
